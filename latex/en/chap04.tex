\chapter{Testing} \label{chap_testing}
...
\end{equation}
then \eqref{vmin_implies} fails.  Are there cones for which \eqref{not_pointed} fails?  It turns out that there is a useful class of cones called \textit{pointed} having this property.
...
\Cref{min_vcone_generators} gives us a way to characterize the minimal sets generating pointed V-Cones.  Clearly, there is not a unique minimal set generating any V-Cone, since any positive scaling of any of the vectors generating the cone results in the same cone.  However, as one is wont to do upon encountering such trifles, we can relax the requirement of unicity to equivalence, in the following way.
...
\begin{proof}
	...
	$(3 \Rightarrow 1)$.  Since $V$ is the vertex set of $\conv(V)$, if $\v\in V$ is also in $\VP{U}{V\setminus\set{\v}}$, then $\v$ can be written with a non-negative contribution from $\cone(U)$, so $V$ is not $U$-free.  Next let $\u\in U$, and $U'=U\setminus\set{\u}$.  We must find a point in $\VP{U}{V}$ that is not in $\VP{U'}{V}$.  Because $\u \not\in\cone(U')$, there is an $\x$ that satisfies: $\x^T U' \leq \0$, and $\x^T \u > 0$.  By \Cref{max_linear_func}, there is some maximum value $c$ such that $(\forall \x\in\conv(V)) \x^T \u \leq c$.  This means that $\set{\x^T \y : \y \in \VP{U'}{V}}$ is upper-bounded by $c$.  But the set $\set{\x^T \y : \y \in \VP{U}{V}}$ is unbounded, since $\x^T \u > 0$.  So we can conclude that $\VP{U'}{V} \subset \VP{U}{V}$.  We conclude that $(U,V)$ are minimal.
\end{proof}
...
\begin{Def}[Full-Dimensional Polyhedra]
...
\begin{proof}
  \Cref{dual_homogenization_cone} shows that the equality of the H-Cones is predicated on the equality of the V-Cones $\cone\psmb -\b \\ A \psme$ and $\cone\psmb -\b' \\ A' \psme$.  \Cref{dual_cone_min} shows that we end up with the situation that either $\cone\psmb -\b \\ A \psme = \cone\psmb -\b' \\ A' \psme$ with $\cone\psmb -\b \\ A \psme$ minimal, in which case $\psmb -\b \\ A \psme \sqsubseteq \psmb -\b' \\ A' \psme$, or $\cone\psmb -\b & -1 \\ A & 0\psme$ is minimal.  In this case, since for no $A_i \in A$ is $A_i \simeq \0$, we conclude that $\psmb -\b \\ A \psme \sqsubseteq \psmb -\b' \\ A' \psme$
\end{proof}
...
\section{Full-Dimensional and Pointed Polyhedra}
...
\subsection{Full-Dimensional V-Polyhedra}

Which V-Cones are full dimensional?  Well, a V-Cone is full dimensional if there is no non-zero $\y\in\R^d$ and $c\in\R$ such that $\ip{\y}{\x} = c$ for all $\x\in\cone(U)$.  First note that if such a $\y$ and $c$ exist, then $c = 0$.  This is because every hyperplane which may contain $\cone(U)$ must contain the origin.  If $U$ has $d$ {\LI} vectors, then clearly $\cone(U)$ is full-dimensional.  Suppose that $U$ does not have $d$ {\LI} vectors, then there is some vector $\y$ such that $(\forall\v\in U)\,\ip{\y}{\v}=0$.  Then $\y^T U\t=\0^T\t=0$, so $\cone(U)$ is not full-dimensional.  So much for V-Cones.  The situation is similar for V-Polyhedra.  In $\cone(U) + \conv(V)$ there are two sets adding to the dimensionality of the polyhedron, $U$ and $V$.  Let $\bar V= \{\v - \v'\st \v,\v'\in V\}$.  Then we must look for $d$ {\LI} vectors in $U\cup\bar V$.  Basically, the cone adds to the dimensionality in the same way as before, but the vectors from $\conv(V)$ are not simply the points of $V$.  More formally, say that there is some $\y$ such that $\y^T U = \y^T \bar V = 0$.  Let $\v'\in V$, and observe that:
\[ \y^T\bar V = 0 \Rightarrow 
      (\forall\v\in V) \ip{\y}{\v-\v'}=0 \Rightarrow 
      (\forall\v\in V)\ip{\y}{\v} = \ip{\y}{\v'}\]
Let $c = \ip{\y}{\v'}$.  Then for any member of $\cone(U)+\conv(V)$
\[\ip{\y}{\sumi t_i\u_i + \sumj\lambda_j\v_j} = 0 + \sumj \lambda_j c = c \]

\subsection{Pointed H-Polyhedra}

Which H-Cones are pointed?  As previously discussed, if a Cone has a vertex, it is the origin.  So, if $\HC{A}$ is pointed, then no $\u\neq\0$ satisfies $A\u=\0$.  Conversely, if $\HC{A}$ is not pointed, then for some $\u\neq\0$ we have $A\u\leq\0$ and $-A\u\leq\0$, so $A\u=\0$.  In either case, we see that the rows of $A$ have a non-trivial orthogonal subspace, so it must be that there are not $d$ {\LI} rows of $A$.  What about for H-Polyhedra?  First, assume that $\HP{A}{\b}$ is non-empty, otherwise there are certainly no vertices.  As before, if there is some $\u\neq\0$ such that $A\u=\0$, then this $\u$ prevents any point from being a vertex.  Now suppose that $\HP{A}{\b}$ has a vertex $\v$.  We claim that there are $d$ {\LI} rows $A_i$ from $A$ such that $\ip{A_i}{\v}=b_i$.  If there were fewer, then we take some $\u$ that is in the orthogonal subspace of the $A_i$ for which $\ip{A_i}{\v}=b_i$, and $A(\v\pm\epsilon\u)\leq\b$ for some small $\epsilon$.  So, a non-empty $\HP{A}{\b}$ has a vertex if and only if $A$ has full row rank.  This result is nice in that it shows the existence of a vertex doesn't depend on $\b$ (for the most part).  If you imagine continuously changing $\b$, you are just ``sliding'' the half-spaces which you are intersecting ``back and forth.''  Vertices may be split or joined, but you cannot completely get rid of all the vertices from the polyhedron (unless you choose a $\b$ which makes the polyhedron empty).  This is an example of an ``intuitive'' result about polyhedra for which the proof is somewhat indirect.

\subsection{Summary}

Here we present a table summarizing the situation for full-dimensionality and pointedness of polyhedra.  $\t$ is a non-negative vector, and abbreviate {\LI} as LI. $\bar V$ denotes $\{\v-\v'\st\v,\v'\in V\}$.\\

\renewcommand{\arraystretch}{1.3}
\begin{tabular}{|l|l|l|}\hline
        & Pointed & Full-Dimensional \\ \hline
$\cone(U)$ & $U\t=\0 \Rightarrow \t=\0$ & $d$ LI vectors in $U$ \\  \hline
$\cone(U)+\conv(V)$ & $U\t=\0 \Rightarrow \t=\0$ & $d$ LI vectors in $U\cup \bar V$ \\ \hline
$\HC{A}$ & $d$ LI row vectors in $A$ & $\t^T A=\0 \Rightarrow \t= \0$ \\ \hline
$\HP{A}{\b}$ & $d$ LI row vectors in $A$ & $\t^T A=\0 \Rightarrow \t^T\b > 0$ \\ \hline
\end{tabular}
...
\subsection{\lsti{struct hcone_test_case}}
\lsthconetestcasea
\subsection{\lsti{struct vcone_test_case}}
\lstvconetestcasea
\subsection{\lsti{struct hpoly_test_case}}
\lsthpolytestcaseb
\subsection{\lsti{struct vpoly_test_case}}
\lstvpolytestcaseb

\section{\filename{test\_functions.cpp}}

\subsection{\lsti{double operator*(Vector,Vector)}}
The dot-product and norm (in terms of dot product).
\lstoperator
\subsection{\lsti{double norm(Vector)}}
\lstnorm

\subsection{\lsti{bool approximately_zero(double)}}
...
\subsection{\lsti{bool ray_satisfied(Matrix,Vector)}}
Test $A\v \leq \0$
\lstraysatisfiedb

\subsection{\lsti{bool rays_satisfied(Matrix,Matrix)}}
Test $AV \leq \0$
\lstrayssatisfied

\subsection{\lsti{bool vec_satisfied(Vector,Vector)}}
Test $\ip{A_i}{\v} \leq b_i$
\lstvecsatisfieda

\subsection{\lsti{bool vec_satisfied(Matrix,Vector)}}
Test $A\v \leq \b$
\lstvecsatisfiedb

\subsection{\lsti{bool vecs_satisfied(Matrix,Matrix)}}
Test $AV \leq \b$
\lstvecssatisfied

\subsection{\lsti{bool equivalent_cone_rep(Matrix,Matrix,Matrix)}}
...
\subsection{\lsti{bool equivalent_hpoly_rep(Matrix,VPoly,VPoly)}}
Given an H-Polytope $P = \HP{A}{\b} = \VP{U}{V}$ where $U$ and $V$ are minimal, and a pair $(U',V')$, determines if $P = \VP{U'}{V'}$.
\lstequivalenthpolyrep

\subsection{\lsti{bool equivalent_vpoly_rep(VPoly,Matrix,Matrix)}}
