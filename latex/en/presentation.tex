...
\begin{frame}{V-Cones are H-Cones}{Fourier-Motzkin Elimination}
	...
	\begin{itemize}
		\item<2-> First, partition the rows as follows:\\
		      $Z$: rows with $k$-th column is $0$ \\
		      $P$: rows with $k$-th column is positive\\
		      $N$: rows with $k$-th column is negative
...
\begin{frame}{H-Cones are V-Cones}{Fourier-Motzkin Elimination}
	Suppose we are intersecting row $k$ from $\cone(U)$.
	\begin{itemize}
		\item<2-> First, partition the columns as follows:\\
		      $Z$: columns with $k$-th row is $0$ \\
		      $P$: columns with $k$-th row is positive\\
		      $N$: columns with $k$-th row is negative
